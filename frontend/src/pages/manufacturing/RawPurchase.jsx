...
export default function RawPurchase() {
  ...
  const { data, pagination, loading, saving, params, setPage, setSearch, updateParams, createItem, updateItem, deleteItem, fetchData } =
  ...
  // Prefetch on page mount (cached) so modal dropdown is ready
  const vendors = useFetchOptions('/manufacturing/vendors');
  ...
  return (
    <div>
      ...
      <Modal isOpen={modalOpen} onClose={() => { if (!saving) { setModalOpen(false); setEditRow(null); } }} title={editRow ? 'Edit Raw Purchase' : 'Add Raw Purchase'}>
        <form onSubmit={handleSubmit} className="space-y-4" key={editRow?._id || 'new'}>
          <div>
            ...
            {vendors.length === 0 ? (
              ...
              <select name="vendor" required className="input-field" defaultValue={editRow?.vendor?._id || editRow?.vendor || ''} disabled={saving}>
                <option value="">Select vendor</option>
                {vendors.map((v) => <option key={v._id} value={v._id}>{v.name}</option>)}
              </select>
            )}
          </div>
          <div><FieldLabel required>Lot Number</FieldLabel><input name="lotNumber" required defaultValue={editRow?.lotNumber} className="input-field" disabled={saving} /></div>
          <div className="form-grid-2">
            <div><FieldLabel required>Quantity (KG)</FieldLabel><input name="quantity" type="number" step="0.01" min="0.01" required defaultValue={editRow?.quantity} className="input-field" disabled={saving} /></div>
            <div><FieldLabel required>Rate (₹ per KG)</FieldLabel><input name="purchaseRate" type="number" step="0.01" min="0" required defaultValue={editRow?.purchaseRate} className="input-field" disabled={saving} /></div>
          </div>
          <div><FieldLabel required>Date</FieldLabel><input name="date" type="date" required defaultValue={defaultDate} className="input-field" disabled={saving} /></div>
          <button type="submit" disabled={saving || vendors.length === 0} className="btn-primary w-full">
            {saving ? 'Saving...' : `${editRow ? 'Update' : 'Save'} Purchase`}
          </button>
        </form>
      </Modal>
