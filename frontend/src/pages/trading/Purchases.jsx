              ...
              <tbody>
                {data.length === 0 ? <tr><td colSpan={8}><EmptyState /></td></tr> : data.map((r) => (
                  <tr key={r._id}>
                    ...
                    <td><EntryActions onEdit={() => openEdit(r)} onDelete={(reason) => deleteItem(r._id, reason)} deleteTitle="Delete purchase" editTitle="Edit purchase" itemLabel={`${r.serialNumber || ''} · ${r.party?.name || 'Vendor'} · ${r.item?.name || 'Item'} · ${formatNumber(r.quantity)} · ${formatDate(r.date)}`} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
